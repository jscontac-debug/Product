 ...
 * informes.js
 * -----------------------------------------------------------------------
 * Pantalla "Informes": indicadores agregados de una semana concreta del
 * cuadrante generado (horas, coste, productividad, cobertura, aperturas,
 * cierres, domingos, vacaciones). El coste solo se muestra si la tienda
 * lo tiene activado ("Coste de personal" en la pantalla Tienda).
 * -----------------------------------------------------------------------
 */

import { leer, KEYS, DIAS, DIAS_LABEL, numeroSemanaISO, formatoFechaCorta } from './storage.js';
import { costeHoraEmpleado } from './motor.js';
...
export async function render(container) {
  ...
  pintar(container, contenedor, semanasKeys, personal, cobertura, tienda);
}

function pintar(container, contenedor, semanasKeys, personal, cobertura, tienda) {
  const semana = contenedor.semanas[semanaSeleccionada];
  const costeHabilitado = tienda.costeHabilitado !== false;
  const totales = calcularTotales(semana, personal, costeHabilitado);
  const coberturaPct = calcularCoberturaMedia(semana, cobertura);
  ...
  container.innerHTML = `
    <div class="screen">
      <div class="screen-header">
        <div>
          <h1>Informes</h1>
          <p>Indicadores de la semana ${w.anio}-${String(w.semana).padStart(2, '0')} (${formatoFechaCorta(semanaSeleccionada)}).</p>
        </div>
        <div class="field" style="min-width:200px;">
          <label for="sel-semana-inf">Semana</label>
          <select id="sel-semana-inf">
            ${semanasKeys.map(k => {
      ...
      <div class="grid ${costeHabilitado ? 'grid-4' : 'grid-3'}" style="margin-bottom:16px;">
        ...
        ${costeHabilitado ? `<div class="stat"><div class="stat__value">${totales.costeTotal.toFixed(0)} €</div><div class="stat__label">Coste estimado</div></div>` : ''}
      ...
      <div class="card">
        <h3>Horas${costeHabilitado ? ' y coste' : ''} por empleado</h3>
        <div class="table-wrap">
          <table>
            <thead>
              <tr>
                <th>Empleado</th><th>Horas</th><th>Contrato</th>
                ${costeHabilitado ? '<th>Coste estimado</th>' : ''}
                <th>Aperturas</th><th>Cierres</th><th>Domingos</th>
              </tr>
            </thead>
            <tbody>
              ${personal.map(p => filaEmpleado(p, semana, costeHabilitado)).join('') || `<tr class="empty-row"><td colspan="${costeHabilitado ? 7 : 6}">Sin empleados</td></tr>`}
            </tbody>
          </table>
        </div>
      </div>
      ...
      <div class="card">
        ...
        <div class="table-wrap">
          <table>
            ...
            <tbody>${cobertura.map(filaCobertura(semana)).join('') || '<tr class="empty-row"><td colspan="4">Sin franjas configuradas</td></tr>'}</tbody>
          </table>
        </div>
      </div>
  ...
  document.getElementById('sel-semana-inf').addEventListener('change', (ev) => {
    semanaSeleccionada = ev.target.value;
    pintar(container, contenedor, semanasKeys, personal, cobertura, tienda);
  });
}

/** Calcula horas, coste y productividad agregados de una semana. */
function calcularTotales(semana, personal, costeHabilitado) {
  const mapaPersonal = {};
...
/** Calcula el porcentaje medio de cobertura alcanzado frente al deseado, en todas las franjas de una semana. */
function calcularCoberturaMedia(semana, cobertura) {
...
function filaEmpleado(p, semana, costeHabilitado) {
  const r = semana.resumenEmpleado[p.id] || { horas: 0, aperturas: 0, cierres: 0, domingos: 0 };
  const coste = r.horas * costeHoraEmpleado(p);
  return `
    <tr>
      <td>${esc(p.nombre)}</td>
      <td>${window.UI.formatoHoras(r.horas)}</td>
      <td>${p.horasSemanales} h</td>
      ${costeHabilitado ? `<td>${coste.toFixed(2)} €</td>` : ''}
      <td>${r.aperturas}</td>
      <td>${r.cierres}</td>
      <td>${r.domingos}</td>
    </tr>
